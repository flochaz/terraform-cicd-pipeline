...
resource "aws_codebuild_project" "hello_world_codebuild_project" {
  name          = "hello_world"
  description   = "Code build project for modules"
  build_timeout = "120"
  service_role  = aws_iam_role.codebuild_role.arn
  ...
  logs_config {
    ...
    s3_logs {
      status   = "ENABLED"
      location = "${aws_s3_bucket.codebuild_bucket.id}/modules_codebuild_project/build_logs"
    }
  }
